...
void loop()
{
  ...
  { // If data ready bit set, then read magnetometer data
    ...
    for(int i = 0 ; i < 5;i++)
    {
      int16_t tempmagCount[3]; 
      readMagData(tempmagCount);  // Read the x/y/z adc values
      magCount[0] += tempmagCount[0];magCount[1] += tempmagCount[1];magCount[2] += tempmagCount[2];
      delay(10);
    }
    magCount[0] /= 5.0;magCount[1] /= 5.0;magCount[2] /= 5.0;
    
    mRes = 0.73; // Conversion to milliGauss, 0.73 mG/LSB in highest resolution mode

    // Calculate the magnetometer values in milliGauss
    // Include factory calibration per data sheet and user environmental corrections
    mx = (float)magCount[0]*mRes - calibData.magbias[0]; // get actual magnetometer value, this depends on scale being set
    my = (float)magCount[1]*mRes - calibData.magbias[1];
    mz = (float)magCount[2]*mRes - calibData.magbias[2];
  }
  ...
  delt_t = millis() - count;
  if (delt_t > 50) { 
  int16_t mag_x = (int16_t)mx, mag_y=(int16_t)my, mag_z=(int16_t)mz;
  int16_t acc_x=(int16_t)ax, acc_y=(int16_t)ay, acc_z=(int16_t)az;
  double roll1;
  double pitch1;
  double azimuth;
  double X_h, Y_h;
  ...
  /* Calculate pitch and roll, in the range (-pi,pi) */
    pitch1 = atan2((double)-acc_x, sqrt((long)acc_z*(long)acc_z + (long)acc_y*(long)acc_y));
    roll1 = atan2((double)acc_y, sqrt((long)acc_z*(long)acc_z  + (long)acc_x*(long)acc_x));
    X_h = (double)mag_x*cos(pitch1) + (double)mag_y*sin(roll1)*sin(pitch1) + (double)mag_z*cos(roll1)*sin(pitch1);
    Y_h = (double)mag_y*cos(roll1) - (double)mag_z*sin(roll1);
    ...
    //yaw   = atan2(2.0f * (q[1] * q[2] + q[0] * q[3]), 1 - (2.0f * (q[2] * q[2] + q[0] * q[0])));
...
//    float q2sqr = q[2] * q[2];
//    float t0 = -2.0 * (q2sqr + q[3] * q[3]) + 1.0;
//    float t1 = +2.0 * (q[1] * q[2] + q[0] * q[3]);
//    float t2 = -2.0 * (q[1] * q[3] - q[0] * q[2]);
//    float t3 = +2.0 * (q[2] * q[3] + q[0] * q[1]);
//    float t4 = -2.0 * (q[1] * q[1] + q2sqr) + 1.0;
//
//    t2 = t2 > 1.0 ? 1.0 : t2;
//    t2 = t2 < -1.0 ? -1.0 : t2;
//
...
void calibrateHMC5883()
{
  ...
  for (int i = 0; i < 500; i++)
  {
    ...
    mx = (float)magCount[0]*0.73;
    my = (float)magCount[1]*0.73;
    mz = (float)magCount[2]*0.73;
    if (mx < minx)
      minx = mx;
    if (my < miny)
      miny = my;
  ...
  calibData.magbias[0] = (maxx + minx) / 2;
  calibData.magbias[1] = (maxy + miny) / 2;
  calibData.magbias[2] = (maxz + minz) / 2;
  Serial.println(".");
  ...
  Serial.print("x offset: "); Serial.println(calibData.magbias[0]);
  Serial.print("y offset: "); Serial.println(calibData.magbias[1]);
  Serial.print("z offset: "); Serial.println(calibData.magbias[2]);
